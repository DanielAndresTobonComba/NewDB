...
*/
select format( sum(total) , 0 ) as total_pedidos
from pedido ;
...
select format( avg(total) , 0 ) as total_pedidos
from pedido ;
...
select count(*) as Total_comercial
from ( 
select c.nombre 
from comercial as c 
left join pedido as p on p.id_cliente = c.id
group by c.nombre) as tabla;	
...
select  max(categoría) as Categoria_Maxima
from cliente ;
...
select c.id , c.nombre ,  concat(c.apellido1 , " " , c.apellido2) as Apellido , max(p.total)  
from cliente as c  
left join pedido as p on p.id_cliente = c.id
